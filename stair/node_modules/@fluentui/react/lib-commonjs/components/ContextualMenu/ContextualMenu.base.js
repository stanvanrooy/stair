...
exports.ContextualMenuBase = React.memo(React.forwardRef(function (propsWithoutDefaults, forwardedRef) {
    var _a, _b;
    var _c = Utilities_1.getPropsWithDefaults(DEFAULT_PROPS, propsWithoutDefaults), ref = _c.ref, props = tslib_1.__rest(_c, ["ref"]);
    ...
    var _d = react_hooks_1.useTarget(props.target, hostElement), targetRef = _d[0], targetWindow = _d[1];
    var tryFocusPreviousActiveElement = usePreviousActiveElement(props, targetWindow)[0];
    var _e = useSubMenuState(props, dismiss), expandedMenuItemKey = _e[0], openSubMenu = _e[1], getSubmenuProps = _e[2], onSubMenuDismiss = _e[3];
    var _f = useShouldUpdateFocusOnMouseMove(props), shouldUpdateFocusOnMouseEvent = _f[0], gotMouseMove = _f[1], onMenuFocusCapture = _f[2];
    var _g = useScrollHandler(asyncTracker), onScroll = _g[0], isScrollIdle = _g[1];
    var _h = useSubmenuEnterTimer(props, asyncTracker), cancelSubMenuTimer = _h[0], startSubmenuTimer = _h[1], subMenuEntryTimer = _h[2];
    var responsiveMode = ResponsiveMode_1.useResponsiveMode(hostElement, props.responsiveMode);
    useVisibility(props, targetWindow);
    var _j = useKeyHandlers(props, dismiss, hostElement, openSubMenu), onKeyDown = _j[0], onKeyUp = _j[1], onMenuKeyDown = _j[2], onItemKeyDown = _j[3];
    var _k = useMouseHandlers(props, isScrollIdle, subMenuEntryTimer, targetWindow, shouldUpdateFocusOnMouseEvent, gotMouseMove, expandedMenuItemKey, hostElement, startSubmenuTimer, cancelSubMenuTimer, openSubMenu, onSubMenuDismiss, dismiss), onItemMouseEnterBase = _k[0], onItemMouseMoveBase = _k[1], onMouseItemLeave = _k[2], onItemClick = _k[3], onAnchorClick = _k[4], executeItemClick = _k[5], onItemClickBase = _k[6];
    ...
    var adjustedFocusZoneProps = tslib_1.__assign(tslib_1.__assign({}, focusZoneProps), { className: classNames.root, isCircularNavigation: true, handleTabKey: FocusZone_1.FocusZoneTabbableElements.all, direction: (_b = (_a = props.focusZoneProps) === null || _a === void 0 ? void 0 : _a.direction) !== null && _b !== void 0 ? _b : FocusZone_1.FocusZoneDirection.vertical });
